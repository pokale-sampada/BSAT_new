...
<!--                                              <div class="row"> -->
                                            ...
                                            <!--  Budget VS Actual MIS start -->
		<div class="col-md-12 col-lg-5">
			<div class="card">
				<div class="card-header">
					<h5>Budget VS Actual MIS</h5>
					<div class="card-header-right">
						<ul class="list-unstyled card-option">
							<li><i class="feather icon-maximize full-card"></i></li>
							<li><i class="feather icon-minus minimize-card"></i></li>
				...
				<div class="card-block">
					<div class="form-group row">
						<label class="block">Scheme </label>
						<div class="col-md-8">
						...
						<div class="col-md-1">
							<input type="button" class="btn btn-primary btn-sm" id="load"
								value="Load" onclick="loadBudMisData()">
						</div>
					</div>
		...
		<!--  Budget VS Actual MIS End -->
		<!--  Product Category MIS start -->
		<div class="col-xl-8 col-md-12">
			<div class="card">
				<div class="card-header">
					<h5>Product Category MIS</h5>
					<!--   <span class="text-muted">For more details about usage, please refer <a href="https://www.amcharts.com/online-store/" target="_blank">amCharts</a> licences.</span> -->
					<div class="card-header-right">
						<ul class="list-unstyled card-option">
							<li><i class="feather icon-maximize full-card"></i></li>
				...
				<div class="card-block">
					<div class="form-group row">
						<div class="col-md-5">
							<label class="block">Depot</label> <select
								class="form-control form-control-sm form-control-primary"
								name="appl_depot_code" id="appl_depot_code" onchange="">
								<!-- <option>--Select--</option> -->
							</select>
						</div>
						<div class="col-md-5">
							<label class="block">Prd Cat</label> <select
								class="form-control form-control-sm form-control-primary"
								name="prd_cat" id="prd_cat" onchange="">
								<!-- <option>--Select--</option> -->
							</select>
						</div>
					...
					<div class="form-group row">
						<div class="col-md-5">
							<label class="block">From Date</label> <input
								class="form-control form-control-sm form-control-primary"
								type="date" id="from_date" name="from_date" value="" onchange="">
						</div>
						<div class="col-md-5">
							<label class="block">To Date</label> <input
								class="form-control form-control-sm form-control-primary"
								value="" type="date" id="to_date" name="to_date" onchange="">
						</div>
						<div class="col-md-1">
							<input type="button" class="btn btn-primary btn-sm" id="load"
								style="margin-top: -1rem;" value="Load" onclick="loadData()">
						</div>
					</div>
					<div id="loading"
						style="width: 100%; height: 130%; margin-left: 2%; margin-top: 0%;">
						<img src="files\assets\images\1loading.gif" alt="BASS"
							style="width: 7%; height: 7%; margin-left: 38%; margin-top: 0%;"
							class="" />
						<h6 style="margin-left: 30%;">Loading Data, Please Wait.....</h6>
					</div>
					<div id="chartContainer1"
						style="height: 400px; max-width: 920px; margin: 0px auto;"></div>
					<!--  <div id="sales-analytics" style="height: 265px;"></div> -->
				</div>
			</div>
		</div>
		<!--  Product Category MIS End -->
...
window.onload = function () {
		...
		success : function(data) {
			...
			$.each(data, function(index, value) {
				...
				if (value.depot_code == "${deptnm}") {
					$('<option selected>').val(value.depot_code).text(
	...
	$.ajax({
		url : '${pageContext.request.contextPath}/getProductCategory',
	...
	success : function(data) {
		...
		$.each(data, function(index, value) {
			...
			if (value.scheme_id == "${scheme_id}") {
				$('<option selected>').val(value.scheme_id)
						.text(scheme_nm_code).appendTo(select);
			} else {
				$('<option>').val(value.scheme_id).text(
...
function loadBudMisData() {
	...
	if (document.getElementById("scheme_name").value == "") {
		...
		$
				.get(
						'${pageContext.request.contextPath}/getBudgetVsActualData',
						{
							scheme_name : scheme_name
						},
						function(data) {
							for (var i = 0; i < data.length; i++) {
								//alert("Data is : "+data[i].expense_before_exception);
									...
									series : [
											{
												...
												data : [
														data[i].expense_before_exception,
														data[i].expense_after_exception ]
											},
											{
												name : 'Varience',
												type : 'bar',
												...
												data : [
														data[i].variance_from_budget_before_exception,
														data[i].variance_from_budget_after_exception ]
											} ]
								};
...
{
	//alert("In load data function"); 
	$('#loading').show();
	var appl_depot_code = document.getElementById("appl_depot_code").value;
	var from_date1 = document.getElementById("from_date").value;
	var to_date1 = document.getElementById("to_date").value;
	var productCategory = document.getElementById("prd_cat").value;

	if (appl_depot_code == "" && from_date1 == "" && to_date1 == ""
			&& productCategory == "") {

		alert("Please select Depot,Product and Dates");
		$('#loading').hide();
	} else {

		var to_date = moment(to_date1).format('DD-MM-YYYY');
		var from_date = moment(from_date1).format('DD-MM-YYYY');
		...
		/*  $.getJSON("https://canvasjs.com/data/docs/btcusd2018.json", function(data) {
		   for(var i = 0; i < data.length; i++){
		     dataPoints1.push({x: new Date(data[i].date), y: Number(data[i].open)});
		     dataPoints2.push({x: new Date(data[i].date), y: Number(data[i].volume_usd)});
		     dataPoints3.push({x: new Date(data[i].date), y: Number(data[i].volume_usd)});
		   }
		   stockChart.render();
		 }); */
		$.get('${pageContext.request.contextPath}/misproduct', {
			appl_depot_code : appl_depot_code,
			from_date : from_date,
			to_date : to_date,
