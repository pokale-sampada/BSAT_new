...
public class OPABatchSchemeAnalysisTrigger implements Runnable {
	...
	public OPABatchSchemeAnalysisTrigger(TilesConfiguration hibernateConfiguration,
	...
	public void run() {
		...
		LYSD = cal.getTime();
		System.out.println("enter in thread OPABatchSchemeAnalysisTrigger");
...
//				System.out.println("calling BPIL_GET_SCH_AN_FLAG");
				CallableStatement cStmt = connection.prepareCall("{call BPIL_GET_SCH_AN_FLAG(?)}");
				cStmt.registerOutParameter(1, Types.VARCHAR);

				ResultSet result = cStmt.executeQuery();
				String flag = cStmt.getString(1);
//				System.out.println("BPIL_GET_SCH_AN_FLAG =" +flag);
				
				cStmt.close();
				if (flag.equals("Y")) {

					ArrayList<Bpil_BatSchAnalysis_Process_Queue> rewprocessqueue = (ArrayList<Bpil_BatSchAnalysis_Process_Queue>) hibernateTemplate
							.find("from Bpil_BatSchAnalysis_Process_Queue where status='Pending' order by scheme_id asc");

					System.out.println("Pending Bpil_BatSchAnalysis_Process_Queue = " + rewprocessqueue.size());

					for (Bpil_BatSchAnalysis_Process_Queue bpil_Process_Queue : rewprocessqueue) {
						...
						String updateschstartquery = "UPDATE BPIL_SCH_ANALYSIS_QUEUE "
						...
						if (checkoutputflag.equals("Y")) {
							...
							String updatespstartquery = "UPDATE BPIL_SCH_ANALYSIS_QUEUE "
							...
							System.out.println("calling BPIL_SCH_AN_INPUT_ENTRY");
							CallableStatement getschinputStmt = connection.prepareCall("{call BPIL_SCH_AN_INPUT_ENTRY(?,?,?)}");
							...
							System.out.println("BPIL_SCH_AN_INPUT_ENTRY flag =" + getschinputflag);
							System.out.println("BPIL_SCH_AN_INPUT_ENTRY exp =" + getschinputexp);
							...
							String updatespendquery = "UPDATE BPIL_SCH_ANALYSIS_QUEUE "
...
//												}
											...
											compInvalStmt.setString(1, "BPIL_SCH_AN_INPUT_ENTRY");
...
//											schrwsortStmt.close();
											...
											String updateschendquery = "UPDATE BPIL_SCH_ANALYSIS_QUEUE "
