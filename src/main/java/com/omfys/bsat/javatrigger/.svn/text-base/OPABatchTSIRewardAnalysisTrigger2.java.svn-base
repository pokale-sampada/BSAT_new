...
import com.omfys.bsat.model.Bpil_BatTSIRwAnalysis_Process_Queue;
import com.omfys.bsat.model.Bpil_BatTSIRwAnalysis_Process_Queue2;
import com.omfys.bsat.model.New_Scheme_mstr;

public class OPABatchTSIRewardAnalysisTrigger2 implements Runnable {
	...
	public OPABatchTSIRewardAnalysisTrigger2(TilesConfiguration hibernateConfiguration,
	...
	public void run() {
		...
		LYSD = cal.getTime();
		System.out.println("enter in thread OPABatchTSIRewardAnalysisTrigger2");
...
//				if (LYSD.equals(new Date())) {
				...
				// System.out.println("calling BPIL_GET_RW_TSI_AN_FLAG2");
				CallableStatement cStmt = connection
						.prepareCall("{call BPIL_GET_RW_TSI_AN_FLAG2(?)}");

				cStmt.registerOutParameter(1, Types.VARCHAR);
				ResultSet result = cStmt.executeQuery();
				String flag = cStmt.getString(1);
				// System.out.println("BPIL_GET_RW_TSI_AN_FLAG2 =" +flag);
				
				cStmt.close();
				if (flag.equals("Y")) {

					ArrayList<Bpil_BatTSIRwAnalysis_Process_Queue2> rwprocessqueue = (ArrayList<Bpil_BatTSIRwAnalysis_Process_Queue2>) hibernateTemplate
							.find("from Bpil_BatTSIRwAnalysis_Process_Queue2 where status='Pending' order by scheme_id asc");

					System.out.println("Pending Bpil_BatTSIRwAnalysis_Process_Queue2 = " + rwprocessqueue.size());

					for (Bpil_BatTSIRwAnalysis_Process_Queue2 bpil_Process_Queue : rwprocessqueue) {

						System.out.println("Starting TSI Rw bat analysis for schid = " + bpil_Process_Queue.getScheme_id()
						...
						System.out.println("calling BPIL_FLUSH_OPA_RW_TSI_AN_OUT");
						CallableStatement checkoutputStmt = connection
								.prepareCall("{call BPIL_FLUSH_OPA_RW_TSI_AN_OUT(?,?)}");
						...
						System.out.println("BPIL_FLUSH_OPA_RW_TSI_AN_OUT =" + checkoutputflag);
						if (checkoutputflag.equals("Y")) {

							 System.out.println("calling BPIL_OPA_RW_TSI_AN_IN_ENTRY2");
//							System.out.println("calling BPIL_RW_TSI_AN_IN_ENTRY2");
							CallableStatement getschinputStmt = connection
									 .prepareCall("{call BPIL_OPA_RW_TSI_AN_IN_ENTRY2(?,?)}");
//									.prepareCall("{call BPIL_RW_TSI_AN_IN_ENTRY2(?,?)}");
							...
							String getschinputflag = getschinputStmt.getString(2);
							 System.out.println("BPIL_OPA_RW_TSI_AN_IN_ENTRY2 =" +getschinputflag);
//							System.out.println("BPIL_RW_TSI_AN_IN_ENTRY2 =" + getschinputflag);
							...
							if (getschinputflag.equals("Y")) {
								...
								System.out.println(
										"rw bat file name = " + schemebatfilename + " schdir name = " + schemedirname);
								...
								if (waitForprocess == 0) {
									...
									compInvalStmt.setString(1, "BPIL_OPA_RW_TSI_AN_IN_ENTRY2");
									...
									System.out.println("calling BPIL_RW_TSI_AN_GIFT_DESC");
									CallableStatement rwgiftStmt = connection
											.prepareCall("{call BPIL_RW_TSI_AN_GIFT_DESC(?)}");
									...
									System.out.println("calling BPIL_RW_TSI_AN_REWARDS_SORT");
									CallableStatement rwsortStmt = connection
											.prepareCall("{call BPIL_RW_TSI_AN_REWARDS_SORT(?)}");
									...
									System.out.println("calling BPIL_UPDATE_RW_TSI_AN_QUEUE2");
									 CallableStatement cStmt2 = connection
											 .prepareCall("{call BPIL_UPDATE_RW_TSI_AN_QUEUE2(?,?)}");
									
									 cStmt2.setInt(1, scheme_id);
									 cStmt2.setInt(2, bpil_Process_Queue.getRw_analysis_queue_id());
									
									 System.out.println("update process " + scheme_id + " " +
									 + bpil_Process_Queue.getRw_analysis_queue_id());
									 ResultSet result2 = cStmt2.executeQuery();
									 System.out.println("update BPIL_UPDATE_RW_TSI_AN_QUEUE2");
