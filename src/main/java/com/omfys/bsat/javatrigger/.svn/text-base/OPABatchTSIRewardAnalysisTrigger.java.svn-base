...
public class OPABatchTSIRewardAnalysisTrigger implements Runnable {
	...
	public OPABatchTSIRewardAnalysisTrigger(TilesConfiguration hibernateConfiguration,
	...
	public void run() {
		...
		LYSD = cal.getTime();
		System.out.println("enter in thread OPABatchTSIRewardAnalysisTrigger");
...
//				if (LYSD.equals(new Date())) {
				...
				// System.out.println("calling BPIL_GET_RW_TSI_AN_FLAG");
				CallableStatement cStmt = connection
						.prepareCall("{call BPIL_GET_RW_TSI_AN_FLAG(?)}");

				cStmt.registerOutParameter(1, Types.VARCHAR);
				ResultSet result = cStmt.executeQuery();
				String flag = cStmt.getString(1);
				// System.out.println("BPIL_GET_RW_TSI_AN_FLAG =" +flag);
				
				cStmt.close();
				if (flag.equals("Y")) {

					ArrayList<Bpil_BatTSIRwAnalysis_Process_Queue> rwprocessqueue = (ArrayList<Bpil_BatTSIRwAnalysis_Process_Queue>) hibernateTemplate
							.find("from Bpil_BatTSIRwAnalysis_Process_Queue where status='Pending' order by scheme_id asc");

					System.out.println("Pending Bpil_BatTSIRwAnalysis_Process_Queue = " + rwprocessqueue.size());

					for (Bpil_BatTSIRwAnalysis_Process_Queue bpil_Process_Queue : rwprocessqueue) {
						...
						if (checkoutputflag.equals("Y")) {
							 ...
							 System.out.println("calling BPIL_OPA_RW_TSI_AN_IN_ENTRY");
//							System.out.println("calling BPIL_RW_TSI_AN_IN_ENTRY");
							CallableStatement getschinputStmt = connection
									 .prepareCall("{call BPIL_OPA_RW_TSI_AN_IN_ENTRY(?,?)}");
//									.prepareCall("{call BPIL_RW_TSI_AN_IN_ENTRY(?,?)}");
							...
							String getschinputflag = getschinputStmt.getString(2);
							 System.out.println("BPIL_OPA_RW_TSI_AN_IN_ENTRY =" +getschinputflag);
//							System.out.println("BPIL_RW_TSI_AN_IN_ENTRY =" + getschinputflag);
							...
							if (getschinputflag.equals("Y")) {
								...
								if (waitForprocess == 0) {
									...
									compInvalStmt.setString(1, "BPIL_OPA_RW_TSI_AN_IN_ENTRY");
									...
									System.out.println("calling BPIL_UPDATE_RW_TSI_AN_QUEUE");
									 CallableStatement cStmt2 = connection
											 .prepareCall("{call BPIL_UPDATE_RW_TSI_AN_QUEUE(?,?)}");
									 ...
									 System.out.println("update BPIL_UPDATE_RW_TSI_AN_QUEUE");
