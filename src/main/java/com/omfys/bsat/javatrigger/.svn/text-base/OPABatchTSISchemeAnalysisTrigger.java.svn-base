...
import com.omfys.bsat.model.Bpil_BatTSISchAnalysis_Process_Queue;
import com.omfys.bsat.model.New_Scheme_mstr;

public class OPABatchTSISchemeAnalysisTrigger implements Runnable {
	...
	public OPABatchTSISchemeAnalysisTrigger(TilesConfiguration hibernateConfiguration,
	...
	public void run() {
		...
		LYSD = cal.getTime();
		System.out.println("enter in thread OPABatchTSISchemeAnalysisTrigger");
...
//				if (LYSD.equals(new Date())) {
				...
				// System.out.println("calling BPIL_GET_SCH_TSI_AN_FLAG");
				CallableStatement cStmt = connection
						.prepareCall("{call BPIL_GET_SCH_TSI_AN_FLAG(?)}");

				cStmt.registerOutParameter(1, Types.VARCHAR);
				ResultSet result = cStmt.executeQuery();
				String flag = cStmt.getString(1);
				// System.out.println("BPIL_GET_SCH_TSI_AN_FLAG =" +flag);
				
				cStmt.close();
				if (flag.equals("Y")) {

					ArrayList<Bpil_BatTSISchAnalysis_Process_Queue> rewprocessqueue = (ArrayList<Bpil_BatTSISchAnalysis_Process_Queue>) hibernateTemplate
							.find("from Bpil_BatTSISchAnalysis_Process_Queue where status='Pending' order by scheme_id asc");

					System.out.println("Pending Bpil_BatTSISchAnalysis_Process_Queue = " + rewprocessqueue.size());

					for (Bpil_BatTSISchAnalysis_Process_Queue bpil_Process_Queue : rewprocessqueue) {
						...
						if (checkoutputflag.equals("Y")) {
							...
							// BPIL_OPA_SCH_TSI_AN_IN_ENTRY");
							System.out.println("calling BPIL_SCH_TSI_AN_IN_ENTRY");
							CallableStatement getschinputStmt = connection
									// .prepareCall("{call
									// BPIL_OPA_SCH_TSI_AN_IN_ENTRY(?,?)}");
									.prepareCall("{call BPIL_SCH_TSI_AN_IN_ENTRY(?,?)}");
							...
							// System.out.println("BPIL_OPA_SCH_TSI_AN_IN_ENTRY
							// =" +getschinputflag);
							System.out.println("BPIL_SCH_TSI_AN_IN_ENTRY =" + getschinputflag);
							...
							if (getschinputflag.equals("Y")) {
								...
								if (waitForprocess == 0) {
									...
									compInvalStmt.setString(1, "BPIL_SCH_TSI_AN_IN_ENTRY");
...
//									System.out.println("calling BPIL_UPDATE_SCH_TSI_AN_QUEUE");
//									 CallableStatement cStmt2 = connection
//											 .prepareCall("{call BPIL_UPDATE_SCH_TSI_AN_QUEUE(?,?)}");
//									
...
//									 System.out.println("update BPIL_UPDATE_SCH_TSI_AN_QUEUE");
//									 
//									 cStmt2.close();
