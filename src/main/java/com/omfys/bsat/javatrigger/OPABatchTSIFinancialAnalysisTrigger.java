...
import com.omfys.bsat.model.Bpil_BatTSIFinAnalysis_Process_Queue;
import com.omfys.bsat.model.New_Scheme_mstr;

public class OPABatchTSIFinancialAnalysisTrigger implements Runnable {
	...
	public OPABatchTSIFinancialAnalysisTrigger(TilesConfiguration hibernateConfiguration,
	...
	public void run() {
		...
		LYSD = cal.getTime();
		System.out.println("enter in thread OPABatchTSIFinancialAnalysisTrigger");
...
//				if (LYSD.equals(new Date())) {
				...
				// System.out.println("calling BPIL_GET_FIN_TSI_AN_FLAG");
				CallableStatement cStmt = connection
						.prepareCall("{call BPIL_GET_FIN_TSI_AN_FLAG(?)}");
				...
				cStmt.close();
				// System.out.println("BPIL_GET_FIN_TSI_AN_FLAG =" +flag);
				if (flag.equals("Y")) {

					ArrayList<Bpil_BatTSIFinAnalysis_Process_Queue> finprocessqueue = (ArrayList<Bpil_BatTSIFinAnalysis_Process_Queue>) hibernateTemplate
							.find("from Bpil_BatTSIFinAnalysis_Process_Queue where status='Pending' order by scheme_id asc");

					System.out.println("Pending Bpil_BatTSIFinAnalysis_Process_Queue = " + finprocessqueue.size());

					for (Bpil_BatTSIFinAnalysis_Process_Queue bpil_Process_Queue : finprocessqueue) {

						System.out.println("Starting TSI Fin bat analysis for schid = " + bpil_Process_Queue.getScheme_id()
						...
						System.out.println("calling BPIL_FLUSH_OPA_FIN_TSI_AN_OUT");
						CallableStatement checkoutputStmt = connection
								.prepareCall("{call BPIL_FLUSH_OPA_FIN_TSI_AN_OUT(?,?)}");
						...
						System.out.println("BPIL_FLUSH_OPA_FIN_TSI_AN_OUT =" + checkoutputflag);
						if (checkoutputflag.equals("Y")) {

							 System.out.println("calling BPIL_OPA_FIN_TSI_AN_IN_ENTRY");
//							System.out.println("calling BPIL_FIN_TSI_AN_IN_ENTRY");
							CallableStatement getschinputStmt = connection
									 .prepareCall("{call BPIL_OPA_FIN_TSI_AN_IN_ENTRY(?,?)}");
//									.prepareCall("{call BPIL_FIN_TSI_AN_IN_ENTRY(?,?)}");
							...
							String getschinputflag = getschinputStmt.getString(2);
							 System.out.println("BPIL_OPA_FIN_TSI_AN_IN_ENTRY =" +getschinputflag);
//							System.out.println("BPIL_FIN_TSI_AN_IN_ENTRY =" + getschinputflag);
							...
							if (getschinputflag.equals("Y")) {
								...
								if (waitForprocess == 0) {
									...
									compInvalStmt.setString(1, "BPIL_OPA_FIN_TSI_AN_IN_ENTRY");
...
//									System.out.println("calling BPIL_FIN_TSI_AN_GIFT_DESC");
//									CallableStatement finrwgiftStmt = connection
//											.prepareCall("{call BPIL_FIN_TSI_AN_GIFT_DESC(?)}");
	//
...
//									finrwgiftStmt.close();
									...
									System.out.println("calling BPIL_FIN_TSI_AN_REWARDS_SORT");
									CallableStatement finrwsortStmt = connection
											.prepareCall("{call BPIL_FIN_TSI_AN_REWARDS_SORT(?)}");
									...
									System.out.println("calling BPIL_UPDATE_FIN_TSI_AN_QUEUE");
									CallableStatement cStmt2 = connection
											 .prepareCall("{call BPIL_UPDATE_FIN_TSI_AN_QUEUE(?,?)}");
									...
									System.out.println("update BPIL_UPDATE_FIN_TSI_AN_QUEUE");
